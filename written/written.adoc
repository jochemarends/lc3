...
Besides these general purpose registers, there's also a 16-bit program counter which is ofter referred to as `PC`.
...
Finally, if the final value is greater than zero, the `P` flag gets set.
...
When executing this program the output is `fedcba` since the hard coded string was `abcdef`.
http://lc3tutor.org offers some more example programs or you could write your own ones.
