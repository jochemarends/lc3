...
The `lc3::cpu::load` function is for loading programs.
It accepts any sequence of 16-bit data that conforms to the `std::ranges::input_range` concept.
The program will get loaded at address `0x0000` but an optional parameter can be passed to load it at a different offset.
...
On second thoughts it does not make much sense to keep the memory inside the CPU since they're different units.
It would make more sense if they both were part of a class that represents the machine as a whole (or  alternatively rename `lc3::cpu` to `lc3::machine`).
Once I've realised this, I already wrote it this way and didn't bother to change it anymore.
But it would be something I would take in consideration next time when writing a virtual machine.
