...
The bitwise AND operation is almost identical to addition operation except for that instead of the `+` operator, the `&` operator should be used.
...
Then if the 11^th^ bit of the encoded instruction is set, a jump operation will be performed, else an 11-bit offset will be added to the program counter.
